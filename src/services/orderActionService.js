...
async function handleRequestDiscount(aiAction) {
  ...
  if (activePromos && activePromos.length > 0) {
    ...
    return (
      `Dạ, hiện tại bên em đang có một số ưu đãi đặc biệt dành cho khách hàng may mắn! 🎉\n\n` +
      `Anh/chị có thể sử dụng các mã sau: ${promoList}. Anh/chị nhớ theo dõi Fanpage của HOMS thường xuyên để không bỏ lỡ các đợt khuyến mãi "sốc" nhất nhé! \n\n` +
      `Anh/chị áp dụng ngay mã này khi chốt đơn để được giảm giá nha!`
    );
  } else {
    return (
      `Dạ, rất xin lỗi anh/chị vì hiện tại các chương trình ưu đãi của bên em vừa kết thúc mất rồi ạ. 😔\n\n` +
      `Tuy nhiên, anh/chị đừng buồn nhé! Hãy nhấn "Theo dõi" (Follow) Fanpage HOMS và bật thông báo để là người đầu tiên biết khi có mã giảm giá mới. Bên em luôn có những bất ngờ dành riêng cho những khách hàng thân thiết đấy ạ!`
    );
  }
}
