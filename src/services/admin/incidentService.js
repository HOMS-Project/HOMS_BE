 ...
 */
const listIncidents = async ({ search, type, status, page = 1, limit = 10 }) => {
  const match = {};
      ...
      data: [
        ...
        { $limit: Math.max(1, limit) },
        {
          $project: {
            ...
            reporterId: { _id: '$reporter._id', fullName: '$reporter.fullName', phone: '$reporter.phone' },
            type: 1,
            description: 1,
            images: 1,
            status: 1,
...
const getIncidentById = async (id) => {
  if (!mongoose.Types.ObjectId.isValid(id)) throw { statusCode: 400, message: 'Invalid incident id' };
...
const resolveIncident = async (id, payload = {}, user = null) => {
  ...
  if (typeof payload.compensationAmount !== 'undefined') incident.resolution.compensationAmount = Number(payload.compensationAmount) || 0;
  // save admin note if provided
  if (typeof payload.note !== 'undefined') incident.resolution.note = payload.note;
  // set resolvedAt when marking resolved, prefer provided resolvedAt
  if (payload.resolvedAt) incident.resolution.resolvedAt = new Date(payload.resolvedAt);
  else if (incident.status === 'Resolved') incident.resolution.resolvedAt = incident.resolution.resolvedAt || new Date();
